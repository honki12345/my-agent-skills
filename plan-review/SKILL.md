---
name: plan-review
description: "구현 계획 문서를 6가지 관점으로 순차 리뷰하고, Codex 승인 인터페이스로 의견/승인을 받아 즉시 반영한 뒤 반복 루프를 수행한다."
---

# Plan Review

구현 계획 문서를 6가지 관점으로 검토하되, 결과를 한 번에 제출하지 않는다.
각 관점마다 요약을 먼저 제시하고 Codex 승인 인터페이스로 `승인 / 승인 안 함`을 받고, 필요 시 자동 제공되는 `Other(다른 지시)` 입력을 반영한 뒤 즉시 문서에 반영한다.
...
- 의견/승인 요청은 반드시 Codex 승인 인터페이스(`request_user_input`)를 사용한다.
- 승인 인터페이스에는 명시 옵션 2개만 제공한다.
  1. `승인 (Recommended)`: 제안된 권장 반영안으로 진행
  2. `승인 안 함`: 이번 제안을 반영하지 않고 다음 단계로 진행
- `Other`는 Codex 인터페이스가 자동 제공하므로 별도 옵션으로 직접 추가하지 않는다.
- `Other(다른 지시)`가 선택되면 수정안을 반영한 후 같은 승인 인터페이스로 재승인을 받는다.
- 승인 인터페이스를 사용할 수 없는 모드라면 일반 텍스트 선택을 받지 말고, 승인 인터페이스가 가능한 모드로 전환 요청 후 대기한다.
...
2. 분석 결과의 `[PROPOSED_UPDATES]`를 확인한다.
3. `[PROPOSED_UPDATES]`가 `없음`이면:
   - 해당 관점을 `PASS_NO_ACTION`으로 기록한다.
   - 사용자 선택 단계 없이 다음 관점으로 진행한다.
4. `[PROPOSED_UPDATES]`가 있으면:
   - 사용자에게 아래 형식으로 **요약**을 제시한다.
   - Codex 승인 인터페이스로 `승인 / 승인 안 함`을 받는다. (`Other`는 자동 제공됨)
   - `승인`: 권장 반영안을 즉시 `current_doc`에 반영한다.
   - `승인 안 함`: 반영하지 않고 다음 관점으로 진행한다.
   - `Other(다른 지시)`: 지시를 반영한 수정안을 다시 제시하고 동일 인터페이스로 재승인을 받는다.
...
3. `Other(자동 제공)`: 사용자 지시를 반영해 수정안 재승인

`Other(다른 지시)`를 선택하면 추가 지시를 받은 뒤 반영 초안을 다시 보여주고 동일 승인 인터페이스로 최종 확인을 받는다.
...
**승인 요청**
- Codex 승인 인터페이스에서 `승인 / 승인 안 함`을 선택하거나 `Other(다른 지시)`를 입력
```
...
2. 계획에서 언급한 파일/모듈/패턴의 실제 존재 여부를 확인한다.
3. 의존성/설정 누락과 영향 범위를 평가한다.
...
2. 개발자에게 물어볼 닫힌 질문 또는 선택지 질문을 작성한다.
3. 각 질문의 리스크와 우선순위를 부여한다.
4. 불확실성이 없으면 "질의 필요 없음"을 명시한다.
...
|---|-------------|-------------|-------------------|----------|
| 1 | {항목} | {질문} | {리스크} | CRITICAL/HIGH/MEDIUM/LOW |

[PROPOSED_UPDATES]
- 답변 반영 방식 제안 (INLINE 또는 ADDENDUM)
- 제안이 없으면 `없음`으로 명시
```

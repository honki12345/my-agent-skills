...
수동으로 선택/승인을 물어야 하는 모든 단계에서 반드시 `권장안`과 `이유`를 함께 제시한다.
...
- 승인 요청을 제시할 때는 항상 `권장안`과 `권장 이유`를 함께 제시한다.
...
- 관점 6 질문을 제시할 때도 질문마다 `권장 선택지`와 `권장 이유`를 함께 제시한다.
...
4. `[PROPOSED_UPDATES]`가 있거나 (관점 6이고 `[QUESTIONS]`가 존재하면):
   ...
   - 승인 요청 또는 질문 제시 시마다 `권장안`과 `권장 이유`를 포함한다.
...
`권장안`과 `권장 이유`를 먼저 제시한 뒤 아래 선택을 받는다.

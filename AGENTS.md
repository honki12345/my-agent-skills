...
| 날짜 | Skill | 설명 |
|------|-------|------|
| 2026-02-15 | `gh-review-triage` | PR 코멘트별 초안 판정 + 수동 텍스트 승인 루프 순차 확정 + 근거/반영 방법 제시 |
| 2026-02-14 | `debug-loop` | 버그 재현 우선 + git/문서 기반 가설 검증 + 수정/로깅 반복 루프 |
| 2026-02-14 | `implement-plan` | 계획 문서 기반 구현 + 이탈 시 승인 게이트 + 테스트 통과 시도 |
| 2026-02-14 | `plan-review` | 구현 계획 문서 6가지 관점 순차 요약 리뷰(테스트 계획 현실성/커버리지 포함) + 수동 텍스트 승인 루프 기반 즉시 반영 + 반복 루프 |
...
├── gh-review-triage/
│   └── SKILL.md            # PR 코멘트 순차 판정 + 수동 텍스트 승인 루프 확정
├── git-master/

...
description: GitHub Pull Request 링크를 입력받아 리뷰 코멘트(라인 코멘트/리뷰 코멘트/일반 코멘트)를 수집하고, 코멘트별 초안 판정을 만든 뒤 Codex 승인 인터페이스로 순차 확정한다. 최종적으로 반영 여부 근거와 반영 방법/검증 방법을 제시할 때 사용한다.
---
...
- 명시 옵션은 아래 2개만 제공한다.
  1. `승인 (Recommended)`: 현재 코멘트의 권장 판정/반영안으로 진행
  2. `승인 안 함`: 현재 코멘트 권장안을 확정하지 않음
- `Other`는 Codex가 자동 제공하므로 옵션에 직접 추가하지 않는다.
- `Other(다른 지시)`가 입력되면 지시를 반영해 코멘트 초안을 수정하고 같은 인터페이스로 재승인을 받는다.
...
3. Codex 승인 인터페이스로 `승인 / 승인 안 함`을 받는다. (`Other` 자동 제공)
4. 선택 처리:
   - `승인`: 현재 초안을 최종 판정으로 확정
   - `승인 안 함`: 현재 코멘트를 `미반영(사용자 미승인)`으로 확정
   - `Other(다른 지시)`: 지시를 반영해 초안 수정 후 동일 인터페이스로 재승인
...
승인 게이트 질문도 같은 인터페이스를 사용한다.

- 명시 옵션: `승인 (Recommended)`, `승인 안 함`
- `Other(다른 지시)` 입력 시 대안안을 갱신해 재질의
...
- 위치: {file:line 또는 N/A}
- 초안 판단: {반영 / 미반영 / 미반영(판단 보류)}
- 사용자 결정: 승인 / 승인 안 함 / Other 지시 반영 후 승인
- 최종 판단: 반영 / 미반영
- 근거:
  - {근거 1}
  - {근거 2}

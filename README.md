...
| `plan-review` | 구현 계획 문서 6관점 순차 리뷰 + 수동 텍스트 승인 루프 + 관점6 질문 1개씩 선택 루프 + 반복 안정화 | `/plan-review` 수동 호출 | 2026-02-14 |
...
현재 브랜치의 변경사항을 기준으로 어떤 문서를 갱신해야 하는지 판단하고, 필요한 문서를 직접 수정하는 스킬입니다.

### 주요 내용

- `git diff <base>...HEAD` 기반 변경 파일 수집
- 경로/영역 기준으로 문서 영향 대상 매핑
- 대상 문서를 직접 수정하고 근거(`Sources`)와 변경 요약 정리
...
- 불확실성/애매성 관점은 Summary 후 질문을 1개씩 제시하고 `선택: ...`으로 순차 확정

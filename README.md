...
| Skill | 설명 | 트리거 | 생성일 |
|-------|------|--------|--------|
| `gh-review-triage` | PR 코멘트별 초안 판정 + Codex 승인 인터페이스 순차 확정 + 반영 방법 제시 | PR 링크로 리뷰 코멘트 판정을 요청할 때 | 2026-02-15 |
...
| `plan-review` | 구현 계획 문서 6관점 순차 요약 리뷰(테스트 계획 현실성/커버리지 포함) + Codex 승인 인터페이스 기반 즉시 반영 + 반복 루프 | `/plan-review` 수동 호출 | 2026-02-14 |
...
- 코멘트별 초안 판정을 만든 뒤 Codex 승인 인터페이스로 순차 확정
- `반영` 항목은 수정 대상, 변경 방향, 검증 방법까지 필수 제시
- 충돌 코멘트/범위 초과 리팩토링 필요 시 동일 승인 인터페이스 게이트 적용
...
구현 계획 문서를 6가지 관점으로 순차 리뷰하고, Codex 승인 인터페이스 기반으로 즉시 문서에 반영하는 인터랙티브 리뷰 스킬입니다.

### 주요 내용

- 관점별 요약 후 Codex 승인 인터페이스로 `승인/승인 안 함` 선택 (`Other` 자동 입력 지원)
- 관점별 제안이 없으면 자동 `PASS_NO_ACTION` 처리
- `Other(다른 지시)` 입력 시 수정안 재제시 후 재승인
